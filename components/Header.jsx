    ...
    <>
      ...
      <header className="bg-paper">
        <div className={`${shell} flex min-h-[126px] items-center justify-between border-b border-ink max-[820px]:min-h-[95px]`}>
          <Brand />
          <div className="grid justify-items-end gap-2 text-right text-[0.7rem] uppercase max-[580px]:text-[0.55rem]">
            <span className="max-[820px]:hidden">{today}</span>
            <strong className="bg-yellow px-[9px] py-1.5 text-[0.62rem] max-[580px]:p-[5px] max-[580px]:text-[0.48rem]">Independent daily edition</strong>
          </div>
        </div>
        ...
        <div className="sticky top-0 z-[100] border-b border-ink bg-[rgb(253_252_249/95%)] backdrop-blur-[10px]">
          <nav
            className={`${shell} flex min-h-12 items-center gap-6 text-[0.72rem] font-extrabold tracking-[0.02em] uppercase min-[821px]:overflow-x-auto min-[821px]:[scrollbar-width:none] min-[821px]:[&::-webkit-scrollbar]:hidden [&_a]:whitespace-nowrap [&_a:hover]:text-blue max-[820px]:gap-3 max-[580px]:min-h-11`}
            aria-label="Primary navigation"
          >
            <Link href="/">HOME</Link>

            <div className="contents max-[820px]:hidden">
              {news.categories.map((category) => (
                <Link key={category.slug} href={`/${category.slug}/`}>
                  {category.name}
                </Link>
            ...
            <SearchBox />
            <MobileMenu categories={news.categories} />
          </nav>
        </div>
      </header>
    </>
